...
export default function App() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key.toLowerCase()) {
        ...
        case 'enter':
          e.preventDefault();
          if (ruleAnalysis[currentRuleIndex] && !ruleAnalysis[currentRuleIndex].userDecision) {
            handleRuleDecision(ruleAnalysis[currentRuleIndex].ruleName, 'accepted');
          }
          break;
        case 'c':
          e.preventDefault();
          // Copy documentation - will be handled by the component
          break;
        case 'e':
          e.preventDefault();
          handleJumpToEvidence('Page 4, Line 3b', 'estimate');
          break;
        case 'r':
          e.preventDefault();
          handleJumpToEvidence('Page 2, Section 3', 'roof_report');
          break;
      }
    };
  ...
  }, [mode, currentRuleIndex, ruleAnalysis]);
  ...
  const handleJumpToEvidence = (location: string, type: 'estimate' | 'roof_report') => {
    setHighlightedLocation(location);
    // Clear highlight after a few seconds
    setTimeout(() => setHighlightedLocation(null), 5000);
  };
  ...
  const renderRuleCard = (rule: RuleAnalysisResult) => {
    ...
    // Enhanced props for the new interface
    const enhancedProps = {
      ruleAnalysis: rule,
      onDecision,
    ...
    switch (rule.ruleName) {
      case 'ridge_cap':
        return <HipRidgeCapCard key={rule.ruleName} {...enhancedProps} />;
      case 'starter_strip':
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              {mode === 'review' && (
                <div className="text-xs text-zinc-500 dark:text-zinc-400 space-x-4">
                  <span>Shortcuts: <kbd className="px-1 py-0.5 bg-zinc-200 rounded dark:bg-zinc-700">J/K</kbd> Navigate</span>
                  <span><kbd className="px-1 py-0.5 bg-zinc-200 rounded dark:bg-zinc-700">Enter</kbd> Accept</span>
                  <span><kbd className="px-1 py-0.5 bg-zinc-200 rounded dark:bg-zinc-700">E/R</kbd> Jump to docs</span>
                </div>
              )}
      ...
      <main className="relative">
        {mode === 'overview' ? (
          ...
          /* Review Mode - Enhanced Split Screen */
          <div className="h-[calc(100vh-4rem)] flex bg-zinc-50 dark:bg-zinc-950">
            {/* Left Panel - Business Rules (Enhanced) */}
                  ...
                  <div className="mb-6">
                    <div className="flex items-center justify-between mb-2">
                      <h2 className="text-xl font-semibold text-zinc-900 dark:text-zinc-100">
                        {ruleAnalysis[currentRuleIndex]?.ruleName.replace('_', '/').replace(/\b\w/g, l => l.toUpperCase())} Analysis
                      </h2>
                      <Badge variant="outline" className="text-xs">
                        Rule {currentRuleIndex + 1} of {ruleAnalysis.length}
                      </Badge>
            ...
            {/* Right Panel - Enhanced Contextual Documents */}
