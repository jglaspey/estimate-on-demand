...
export async function extractPhase1Fields(
  ...
  const { maxTokens = 800, timeoutMs = 45000 } = options; // Increased timeout for minimal estimates
...
- CRITICAL: Return ONLY the JSON object, no explanations or additional text`;
    ...
    try {
      ...
      throw new Error(
        `Invalid JSON in Claude response: ${parseError instanceof Error ? parseError.message : 'Parse error'}`
      );
    }
    ...
    // Calculate metrics
    const fieldsFound = Object.values(fields).filter(
      v => v !== null && v !== ''
    ).length;
 ...
 */
function validateAndCleanFields(
  fields: Phase1Fields,
  warnings: string[]
): Phase1Fields {
  ...
  if (cleaned.dateOfLoss && !isValidDate(cleaned.dateOfLoss)) {
    warnings.push(
      `Date of loss format may be non-standard: ${cleaned.dateOfLoss}`
    );
  }
  ...
  if (cleaned.propertyAddress && !isCompleteAddress(cleaned.propertyAddress)) {
    warnings.push(
      'Property address may be incomplete (missing city/state/zip)'
    );
  }
  ...
  if (
    cleaned.claimRep &&
    cleaned.estimator &&
    cleaned.claimRep === cleaned.estimator
  ) {
