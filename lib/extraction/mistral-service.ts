...
export class MistralDocumentService {
   ...
   */
  async processDocuments(filePaths: string[], jobId: string): Promise<ExtractedData> {
    try {
      console.log(`Starting multi-document processing for job ${jobId}: ${filePaths.length} files`);
      
      // Phase 1: Quick priority field extraction from all documents
      const quickExtraction = await this.extractPriorityFields(filePaths, jobId);
      ...
      console.error(`Multi-document processing failed for job ${jobId}:`, error);
   ...
   */
  async extractPriorityFields(filePaths: string[], jobId: string): Promise<QuickExtractionResponse> {
    try {
      console.log(`Phase 1: Quick extraction for job ${jobId}`);
      ...
      for (const filePath of filePaths) {
        ...
        const priorityFields = await this.extractPriorityFieldsFromText(quickOCR.fullText);
        
        // Merge priority fields, keeping highest confidence values
        combinedPriorityFields = this.mergePriorityFields(combinedPriorityFields, priorityFields);
      }
   ...
   */
  async extractFullDocuments(filePaths: string[], jobId: string): Promise<ExtractedData> {
    try {
      console.log(`Phase 2: Full extraction for job ${jobId}`);
   ...
   */
  async processDocument(filePath: string, jobId: string): Promise<ExtractedData> {
    try {
   ...
   */
  private async classifyDocument(fullText: string): Promise<DocumentClassification> {
    const prompt = `
...
Respond with a JSON object:
{
   ...
   */
  private async extractStructuredData(fullText: string, docType: string): Promise<{
    customerInfo: CustomerInfo | null;
    claimInfo: ClaimInfo | null;
    roofingData: RoofingData | null;
    lineItems: Array<any>;
...
Important: Return null for any section where you cannot find reliable data. Be conservative with extractions.`;
   ...
   */
  private parseOCRResponse(content: string): Array<{ pageNumber: number; content: string; confidence: number }> {
   ...
   */
  private async savePageContent(jobId: string, pages: Array<{ pageNumber: number; content: string; confidence: number }>) {
  ...
  private async saveExtractedData(jobId: string, data: ExtractedData) {
    ...
    if (data.customerInfo) {
      ...
      if (data.customerInfo.phone) updateData.customerPhone = data.customerInfo.phone;
      if (data.customerInfo.email) updateData.customerEmail = data.customerInfo.email;
      if (data.customerInfo.address) {
        const addr = data.customerInfo.address;
        updateData.customerAddress = `${addr.street}, ${addr.city}, ${addr.state} ${addr.zipCode}`.trim();
      }
    }
    ...
    if (data.claimInfo) {
      ...
      if (data.claimInfo.policyNumber) updateData.policyNumber = data.claimInfo.policyNumber;
      if (data.claimInfo.dateOfLoss) updateData.dateOfLoss = new Date(data.claimInfo.dateOfLoss);
      if (data.claimInfo.carrier) updateData.carrier = data.claimInfo.carrier;
      if (data.claimInfo.claimRep) updateData.claimRep = data.claimInfo.claimRep;
      if (data.claimInfo.estimator) updateData.estimator = data.claimInfo.estimator;
      if (data.claimInfo.originalEstimate) updateData.originalEstimate = data.claimInfo.originalEstimate;
    }
    
    // Roofing data
    if (data.roofingData) {
      if (data.roofingData.squares) updateData.roofSquares = data.roofingData.squares;
      if (data.roofingData.stories) updateData.roofStories = data.roofingData.stories;
      ...
      if (data.roofingData.ridgeHip) updateData.ridgeHipLength = data.roofingData.ridgeHip;
      if (data.roofingData.valley) updateData.valleyLength = data.roofingData.valley;
      if (data.roofingData.slope) updateData.roofSlope = data.roofingData.slope;
      if (data.roofingData.material) updateData.roofMaterial = data.roofingData.material;
    }
    ...
    console.log(`Saved extraction data and auto-populated ${Object.keys(updateData).length} job fields for ${jobId}`);
  }
  ...
  private async extractTextWithOCRQuick(filePath: string): Promise<{
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Mistral Quick OCR API failed: ${response.status} ${response.statusText}`);
      }
   ...
   */
  private async extractPriorityFieldsFromText(text: string): Promise<PriorityFields> {
    try {
      const prompt = `
...
`;
      ...
      if (!result) throw new Error('No response from Mistral priority field extraction');

      return JSON.parse(result) as PriorityFields;
   ...
   * Merge priority fields, keeping highest confidence values
   */
  private mergePriorityFields(existing: PriorityFields, newFields: PriorityFields): PriorityFields {
    const merged = { ...existing };
    
    for (const [key, newField] of Object.entries(newFields)) {
      if (newField && typeof newField === 'object' && 'value' in newField && 'confidence' in newField) {
   ...
   * Update job record with priority fields immediately
   */
  private async updateJobWithPriorityFields(jobId: string, priorityFields: PriorityFields): Promise<void> {
    try {
      const updateData: any = { updatedAt: new Date() };
      
      if (priorityFields.customer_name?.value) updateData.customerName = priorityFields.customer_name.value;
      if (priorityFields.property_address?.value) updateData.customerAddress = priorityFields.property_address.value;
      if (priorityFields.claim_number?.value) updateData.claimNumber = priorityFields.claim_number.value;
      if (priorityFields.policy_number?.value) updateData.policyNumber = priorityFields.policy_number.value;
      if (priorityFields.date_of_loss?.value) updateData.dateOfLoss = new Date(priorityFields.date_of_loss.value);
      if (priorityFields.carrier?.value) updateData.carrier = priorityFields.carrier.value;
      if (priorityFields.claim_rep?.value) updateData.claimRep = priorityFields.claim_rep.value;
      if (priorityFields.estimator?.value) updateData.estimator = priorityFields.estimator.value;
      if (priorityFields.original_estimate?.value) updateData.originalEstimate = priorityFields.original_estimate.value;
      ...
      console.log(`Updated job ${jobId} with ${Object.keys(updateData).length} priority fields`);
      
    } catch (error) {
      console.error(`Failed to update job ${jobId} with priority fields:`, error);
    }
  }
  ...
  private calculateAverageConfidence(priorityFields: PriorityFields): number {
    const fields = Object.values(priorityFields).filter(field => 
   ...
   */
  private mergeExtractedData(existing: ExtractedData, newData: ExtractedData): ExtractedData {
    return {
      classification: newData.classification.confidence > existing.classification.confidence 
      ...
      rawPageContent: [...existing.rawPageContent, ...newData.rawPageContent]
    };
  }
}
