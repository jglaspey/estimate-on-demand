...
import { ExtractedLineItem, RoofTypeClassification } from '../extraction/claude-line-item-extractor';
...
export interface RidgeCapAnalysisResult {
  ...
  analysisPath: 'laminated_purpose_built' | 'laminated_cut_from_3tab' | '3tab_found' | '3tab_missing' | 'unknown_roof';
  ...
  ridgeCapQuality: 'purpose-built' | 'high-profile' | 'cut-from-3tab' | 'unknown' | null;
...
export class RidgeCapAnalyzer {
   ...
   */
  async analyzeRidgeCapCompliance(input: RidgeCapAnalysisInput): Promise<RidgeCapAnalysisResult> {
    console.log(`🏠 Analyzing ridge cap compliance for job ${input.jobId}`);
    console.log(`   Roof type: ${input.roofType.roofType} (${Math.round(input.roofType.confidence * 100)}% confidence)`);
    console.log(`   Ridge cap items found: ${input.ridgeCapItems.length}`);
    
    // Validate input data
    if (!input.roofType || !input.roofType.roofType) {
      return this.createInsufficientDataResult('Roof type could not be determined');
    }
   ...
   */
  private analyzeLaminatedCompositionShingles(input: RidgeCapAnalysisInput): RidgeCapAnalysisResult {
   ...
   */
  private analyze3TabShingleRoof(input: RidgeCapAnalysisInput): RidgeCapAnalysisResult {
    ...
    const purposeBuiltItems = input.ridgeCapItems.filter(
      item => item.ridgeCapQuality === 'purpose-built' || item.ridgeCapQuality === 'high-profile'
    );
   ...
   */
  private analyzeUnknownRoofType(input: RidgeCapAnalysisInput): RidgeCapAnalysisResult {
    console.log('  📊 Analyzing unknown roof type - using conservative approach');
  ...
  private analyzePurposeBuiltQuantities(
    ...
    const requiredQuantity = this.calculateRequiredRidgeCapQuantity(input.roofMeasurements);
    
    // Get estimate quantity
    const estimateQuantity = primaryRidgeCapItem?.quantity.value || 0;
    
    // Calculate variance
    const varianceAmount = estimateQuantity - requiredQuantity;
    ...
    if (varianceAmount < -5) { // Allow 5 LF tolerance
      varianceType = 'shortage';
      status = 'SUPPLEMENT_NEEDED';
    } else if (varianceAmount > 20) { // Flag excessive overage
      varianceType = 'excess';
    ...
    const costImpact = varianceType === 'shortage' 
      ? Math.abs(varianceAmount) * (primaryRidgeCapItem?.unitPrice || 42.90)
      : 0;
    ...
    return {
      ...
      reasoning: this.generateQuantityAnalysisReasoning(
        input, estimateQuantity, requiredQuantity, varianceAmount, varianceType
      ),
      analysisPath: analysisPath as any,
      ...
      currentSpecification: primaryRidgeCapItem ? {
        ...
        rate: primaryRidgeCapItem.unitPrice ? `$${primaryRidgeCapItem.unitPrice.toFixed(2)}/${primaryRidgeCapItem.quantity.unit}` : null,
        total: primaryRidgeCapItem.rcv ? `$${primaryRidgeCapItem.rcv.toFixed(2)}` : null,
      } : null,
      evidenceReferences: this.generateEvidenceReferences(input),
      documentationNote: this.generateDocumentationNote(input, estimateQuantity, requiredQuantity, varianceAmount),
      supplementRecommendation: varianceType === 'shortage' 
        ? `Add ${Math.abs(varianceAmount)} LF ridge cap coverage`
        : null,
    };
  }
   ...
   */
  private calculateRequiredRidgeCapQuantity(measurements: RoofMeasurements): number {
  ...
  private generateLaminatedCutFrom3TabReasoning(
    ...
    const itemDescriptions = nonCompliantItems.map(item => item.description).join(', ');
    
    return `Laminated composition shingle roofs require purpose-built ridge caps for proper performance and warranty compliance. ` +
  ...
  private generate3TabCutFromReasoning(
    ...
    const itemDescriptions = cutFrom3TabItems.map(item => item.description).join(', ');
    
    return `While 3-tab roofs traditionally used ridge caps cut from 3-tab shingles, current best practices ` +
  ...
  private generateQuantityAnalysisReasoning(
    ...
    if (varianceType === 'shortage') {
      baseReasoning += `Shortage of ${Math.abs(varianceAmount)} LF identified - supplement needed.`;
    } else if (varianceType === 'excess') {
      baseReasoning += `Estimate includes ${varianceAmount} LF overage - adequate with waste factor.`;
    } else {
  ...
  private generateDocumentationNote(
    ...
    if (varianceAmount >= 0) {
      // Adequate coverage
      const overagePercent = requiredQty > 0 ? Math.round((Math.abs(varianceAmount) / requiredQty) * 100) : 0;
      return `Ridge cap coverage verified as adequate. Estimate includes ${estimateQty} LF while measurements show ${requiredQty} LF required. ` +
             `The ${Math.abs(varianceAmount)} LF overage (${overagePercent}%) provides appropriate waste factor for installation. ` +
      ...
      const unitPrice = ridgeCapItem?.unitPrice || 42.90;
      ...
      return `Ridge cap shortage identified. ${measurements.extractedFrom === 'eagleview' ? 'EagleView report' : 'Roof report'} documents ${requiredQty} LF total ridge/hip coverage required ${measurementDetails}. ` +
             ...
             `Additional coverage required: ${additionalLF} LF @ $${unitPrice.toFixed(2)}/LF = $${additionalCost.toFixed(2)}.`;
    }
  }
  ...
  private generateEvidenceReferences(input: RidgeCapAnalysisInput): string[] {
    ...
    input.ridgeCapItems.forEach(item => {
      if (item.source?.page) {
        references.push(`Ridge cap line item: Page ${item.source.page} - "${item.description}"`);
      }
    });
   ...
   */
  private calculateQuantityAnalysisConfidence(input: RidgeCapAnalysisInput): number {
    let confidence = 0.8; // Base confidence
    
    // Higher confidence if we have both ridge and hip measurements
    if (input.roofMeasurements.ridgeLength && input.roofMeasurements.hipLength) {
      confidence += 0.1;
    }
  ...
  private createSupplementNeededResult(
    ...
    const varianceAmount = estimateQuantity - requiredQuantity;
    const costImpact = Math.abs(varianceAmount) * unitPrice;
    ...
    return {
      ...
      requiredQuantity: `${requiredQuantity} LF`,
      variance: varianceAmount >= 0 ? `+${varianceAmount} LF` : `${varianceAmount} LF`,
      varianceAmount,
      varianceType: 'shortage',
      materialStatus: analysisPath.includes('cut_from_3tab') ? 'non-compliant' : 'compliant',
      ...
      currentSpecification: ridgeCapItem ? {
        ...
        rate: ridgeCapItem.unitPrice ? `$${ridgeCapItem.unitPrice.toFixed(2)}/${ridgeCapItem.quantity.unit}` : null,
