...
export default function Upload() {
  ...
  const handleFileSelect = (type: 'estimate' | 'roofReport' | 'supplement') => (file: File) => {
    setFiles(prev => ({ ...prev, [type]: file }))
    setUploadError(null)
  }

  const handleJobDetailChange = (field: keyof JobDetails) => (e: React.ChangeEvent<HTMLInputElement>) => {
    setJobDetails(prev => ({ ...prev, [field]: e.target.value }))
  }

  const handleStartAnalysis = async () => {
    if (!files.estimate && !files.roofReport) {
      setUploadError("Please upload at least one document (estimate or roof report)")
      return
    }
    ...
    try {
      ...
      // Redirect to the job analysis page
    ...
    <div className="min-h-screen bg-zinc-50 p-8 dark:bg-zinc-900">
      <div className="mx-auto max-w-4xl space-y-8">
        <header className="text-center">
          <h1 className="text-3xl font-semibold text-zinc-950 dark:text-white">Upload Documents</h1>
        ...
        <div className="rounded-lg border border-zinc-950/10 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-zinc-900">
          <h3 className="text-lg font-semibold text-zinc-950 dark:text-white">Job Details</h3>
