...
function transformJobToSummary(job: DatabaseJob): JobSummary {
  ...
  const totalSupplementValue = supplementCount * (Math.floor(Math.random() * 800 + 200));
  ...
  useEffect(() => {
    async function fetchJobs() {
      try {
        ...
        const transformedJobs = data.jobs.map((job: DatabaseJob) => transformJobToSummary(job));
      ...
      <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex items-center justify-center h-64">
            <div className="text-lg text-zinc-600 dark:text-zinc-400">Loading jobs...</div>
          </div>
        </div>
      </div>
    );
        ...
        <div className="mb-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
              ...
              <div className="text-sm text-zinc-500 dark:text-zinc-400">Total Jobs</div>
            </div>
            <div className="rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
            ...
            <div className="rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
              <div className="text-2xl font-bold text-blue-600">
                {jobs.filter(j => ['extracting', 'analyzing'].includes(j.status)).length}
              </div>
              <div className="text-sm text-zinc-500 dark:text-zinc-400">Processing</div>
            </div>
            <div className="rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
              <div className="text-2xl font-bold text-zinc-900 dark:text-zinc-100">
                ${jobs.reduce((sum, job) => sum + job.totalSupplementValue, 0).toLocaleString()}
              </div>
              <div className="text-sm text-zinc-500 dark:text-zinc-400">Total Supplements</div>
            </div>
          </div>
        </div>
