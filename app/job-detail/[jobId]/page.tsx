    ...
    const fetchJobData = async () => {
      try {
        ...
        const transformedJob: JobData = {
          ...
          dateOfLoss: job.dateOfLoss ? new Date(job.dateOfLoss).toISOString().split('T')[0] : '2024-01-01',
        ...
        if (job.roofSquares || job.eaveLength || job.rakeLength) {
          const measurements: RoofMeasurements = {
            ...
            numberOfSquares: job.roofSquares,
            predominantPitch: job.roofSlope,
            numberOfStories: job.roofStories,
            totalEaves: job.eaveLength,
            totalRakes: job.rakeLength,
            totalRidges: job.ridgeHipLength,
            totalValleys: job.valleyLength,
          };
    ...
    return (
      <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 flex items-center justify-center">
        <div className="text-center">
          ...
          <Button onClick={() => router.push('/dashboard-real')} variant="outline">
          ...
          <div className="flex h-16 items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <Badge variant="secondary" className="bg-zinc-100 text-zinc-700 border-zinc-200 dark:bg-zinc-800 dark:text-zinc-300 dark:border-zinc-700">
                Job #{jobData?.id?.split('-')[1] || jobData?.id?.slice(-4) || 'Loading...'}
              </Badge>
        ...
        <div className="w-1/2 bg-white border-r border-zinc-200 dark:bg-zinc-900 dark:border-zinc-800">
          <div className="h-full overflow-auto">
            <div className="p-6 space-y-6">
              ...
              {/* Business Rule Card */}
              {ruleAnalysis.length > 0 && renderRuleCard(ruleAnalysis[currentRuleIndex])}
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-6">
            <div className="text-sm">
              <span className="text-zinc-500 dark:text-zinc-400">Total Supplement Impact:</span>
          ...
          <div className="flex items-center gap-2">
            {ruleAnalysis.map((rule, index) => {
              ...
              const IconComponent = rule.status === 'COMPLIANT' ? CheckCircle :
                                  rule.status === 'SUPPLEMENT_NEEDED' ? AlertTriangle :
                                  rule.status === 'INSUFFICIENT_DATA' ? HelpCircle : XCircle;
              ...
              return (
                ...
                >
                  <IconComponent className={`h-4 w-4 ${
                    ...
                    rule.status === 'SUPPLEMENT_NEEDED' ? 'text-red-600' :
                    'text-orange-600'
                  }`} />
                </button>
              );
