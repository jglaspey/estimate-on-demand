...
async function fetchJobData(jobId: string) {
  try {
    // For server-side fetching, we need the full URL
...
export default async function Analysis({ params }: Props) {
  ...
  let _statusMessage = 'Document uploaded, waiting to start processing...';
  let _progressPercent = 10;

  if (hasExtractions) {
    _statusMessage = 'Document text extracted, analyzing business rules...';
    _progressPercent = 40;
  }
  if (hasAnalyses) {
    _statusMessage = 'AI analysis complete, validating business rules...';
    _progressPercent = 70;
  }
  if (hasRuleAnalyses) {
    _statusMessage = 'Business rule analysis complete';
      ...
      <div className='max-w-6xl mx-auto space-y-8'>
        <header>
          <div className='flex items-center justify-between'>
            <div>
              <h1 className='text-3xl font-medium text-foreground'>
                Analysis - {job.fileName || jobId}
              </h1>
              <p className='text-muted-foreground mt-2'>
                Job Status: {job.status} | File Size:{' '}
                {Math.round(((job.fileSize || 0) / 1024 / 1024) * 100) / 100} MB
              </p>
            </div>
        ...
        <div className='grid gap-6'>
            ...
            <div className='bg-card border border-border rounded-lg p-6'>
              <div className='text-center py-8'>
                ...
                <p className='text-muted-foreground mb-4'>
                  Your document is being processed. Text extraction and business
                  rule analysis will begin shortly.
                </p>
          ...
          {hasExtractions && !hasRuleAnalyses && (
            <div className='bg-card border border-border rounded-lg p-6'>
              <div className='text-center py-8'>
                ...
                <p className='text-muted-foreground mb-4'>
                  Text extraction complete. Analyzing business rules and
          ...
          {hasRuleAnalyses &&
            job.ruleAnalyses.map((analysis: any, index: number) => (
              <div
                key={index}
                      ...
                      <p className='text-sm'>
                        {typeof analysis.findings === 'string'
                          ...
                          : analysis.findings?.evidence ||
                            'Analysis findings available'}
                      </p>
                      {analysis.findings?.sourcePages && (
                        <p className='text-xs text-gray-600 mt-1'>
                          Source: Page{' '}
                          {analysis.findings.sourcePages.join(', ')}
                        </p>
                      )}
          ...
          {hasExtractions && !hasRuleAnalyses && (
            <div className='bg-card border border-border rounded-lg p-6'>
              <div className='text-center py-8'>
                ...
                <p className='text-muted-foreground mb-4'>
                  Processing document for compliance with Hip/Ridge Cap, Starter
                  Strip, Ice & Water Barrier, and Drip Edge requirements...
                </p>
                <div className='w-full bg-accent rounded-full h-2 max-w-md mx-auto'>
                  <div
                    className='bg-primary h-2 rounded-full animate-pulse'
