...
Also fixed UI fallback by updating analyze API GET/POST to return full `uiData` for drip edge and ice & water, eliminating cases where the UI displayed "Drip Edge (Missing)" with fresh jobs that actually contain a drip edge line item.
...
- `app/api/jobs/[jobId]/analyze/route.ts` - Populate `uiData.dripEdge` and `uiData.iceAndWater` in both POST and GET responses
...
### API/UI Consistency Fix
- Before: GET `/api/jobs/[jobId]/analyze` only returned `uiData.ridgeCap`; UI sometimes fell back to mock/defaults for drip edge, showing "Missing"
- After: Both POST and GET return `uiData.ridgeCap`, `uiData.dripEdge`, and `uiData.iceAndWater` mapped from live results or saved ruleAnalyses
- Impact: UI always renders correct present/missing state, quantities, costs, and reasoning for drip edge and ice & water across jobs (no more intermittent "missing" when data exists)
...
### Data Verification
### Regression Case: New Job (cmehg3qrw0000saajhq0l6s62)
```
v2.lineItems contains:
- Drip edge 324.99 LF @ $3.41 (code 8)

Analysis:
- dripEdgePresent: true
- gutterApronPresent: false
- complianceStatus: partial
- costImpact: $324.45
- reasoning: Drip edge present for rakes (223 LF); gutter apron missing for eaves (103 LF)

UI: now correctly shows Found/Partial with reasoning (no longer Missing)
```
```json
{
